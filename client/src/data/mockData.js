export const INITIAL_MOCK_DEVICES = [
  {
    id: 1,
    name: "Admin-PC",
    ip: "192.168.1.10",
...
export function generateMockTrafficHistory(points = 20, intervalMins = 5) {
  const history = [];
  const now = new Date();
  ...
  for (let i = points - 1; i >= 0; i--) {
    ...
    const baseDownload = 55 + Math.sin(i * 0.4) * 20 + (Math.random() * 10 - 5);
    const baseUpload = 20 + Math.cos(i * 0.4) * 8 + (Math.random() * 5 - 2.5);
...
// In-browser standalone fallback state
export let localDevices = JSON.parse(JSON.stringify(INITIAL_MOCK_DEVICES));
export let localAlerts = JSON.parse(JSON.stringify(INITIAL_MOCK_ALERTS));
export let localTraffic1h = generateMockTrafficHistory(20, 3);
export let localTraffic6h = generateMockTrafficHistory(24, 15);
export let localTrafficToday = generateMockTrafficHistory(24, 60);
...
export function simulateLocalTick() {
  ...
  const down = online.reduce((acc, d) => acc + d.download, 0);
  const up = online.reduce((acc, d) => acc + d.upload, 0);
  const bw = Math.min(100, Math.round(((down + up) / 150) * 100));

  localTraffic1h.push({
    time: nowStr,
    download: parseFloat(down.toFixed(1)),
    upload: parseFloat(up.toFixed(1)),
    bandwidth: bw
  });

  if (localTraffic1h.length > 25) {
    localTraffic1h.shift();
  }
}
